...
namespace UnityStandardAssets.Vehicles.Car
{
	...
	public class WaypointFollower : MonoBehaviour
    {
        ...
        // 'targets' the car will aim for.
		public int transformIndex = 0;
		// index of the array.
       	private bool m_StopWhenTargetReached = false;
        ...
        // Should the follower apply a throttle value or not
        public bool applyThrottle = true;
        ...
        private void OnEnable()
        {
            ...
            // get the car's rigid body
            m_Rigidbody = GetComponent<Rigidbody>();

            // get the list from the roadBuilder
            m_Targets = RoadBuilder.Singleton.Waypoints;
            ...
            {
                var waypoint = m_Targets[i];
                Vector3 direction = waypoint.transform.position - m_Rigidbody.position;
                float dist = direction.sqrMagnitude;
                float angle = Vector3.Angle(direction, forward);
                if (dist < nearestWaypointDist && angle < 50)
                {
                    transformIndex = i;
            ...
            transformIndex++;
            transformIndex = transformIndex % m_Targets.Count;
        }


        private void FixedUpdate ()
        {
			if(m_Targets.Count == 0 || !m_Driving) {  
			    // Car should not be moving, use handbrake to stop
                m_CarController.Move(0, 0, -1f, 1f);

                // repopulate waypoints for the car if necessary
                m_Targets = RoadBuilder.Singleton.Waypoints;
            }
            else
            {
                ...
                case BrakeCondition.TargetDirectionDifference:
                    {
                        ...
                        float approachingCornerAngle = Vector3.Angle (m_Targets[transformIndex].transform.forward, fwd);
                ...
                case BrakeCondition.TargetDistance:
                    {
						...
						Vector3 delta = m_Targets[transformIndex].transform.position - transform.position;
				...
				Vector3 offsetTargetPos = m_Targets[transformIndex].transform.position;
                ...
                if (Time.time < m_AvoidOtherCarTime) {
					...
					offsetTargetPos += m_Targets[transformIndex].transform.right * m_AvoidPathOffset;
                } else {
                    // no need for evasive action, we can just wander across the path-to-target in a random way,
                    // which can help prevent AI from seeming too uniform and robotic in their driving
					offsetTargetPos += m_Targets[transformIndex].transform.right *
                ...
                if (applyThrottle)
                {
                    if (accel > 0)
                    {
                        m_CarController.Move(
                            steer,
                            accel * m_CarController.MaxThrottleTorque,
                            0f,
                            0f);
                    }
                    else
                    {
                        m_CarController.Move(
                            steer,
                            0f,
                            accel * m_CarController.MaxBrakeTorque,
                            0f);
                    }
                ...
                {
                    m_CarController.ApplySteeringAngle(steer);
                }
				...
				// stop car once it has reached final waypoint
				if (transformIndex == m_Targets.Count - 1) {
                    transformIndex = 0;
                    //this.enabled = false;
                    //m_Driving = false;
				}

				// update to next transform
				if (localTarget.magnitude < m_ReachTargetThreshold && transformIndex < m_Targets.Count) {
                    //Debug.Log("Just Passed: " + m_Targets[transformIndex].name);
                    transformIndex++;
				}
            }
        }
        ...
        {
            //m_Targets[transformIndex].transform = target;
            //m_Driving = true;
        }
    }
}
